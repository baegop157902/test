...
export function createForms(store,stickers,eyedropper) {
  ...
  function fieldPanel(currentSide,group) {
    ...
    for(const image of (Array.isArray(imageFields)?imageFields:imageFields?[imageFields]:[])){
      ...
      const input=document.createElement('input');input.type='file';input.accept='image/png,image/jpeg,image/webp';input.setAttribute('aria-label',(imageLabel||title.textContent)+' 업로드');
      ...
      citationInput.placeholder = '출처가 있나요?';
      
      // 🔥 1번 요청: 일반 이미지 입력폼에도 지워지지 않는 'ⓒ' SVG 마크 고정 삽입
      ...
      citationInput.style.backgroundPosition = '12px center'; // 좌측에서 12px 위치에 고정
      citationInput.style.paddingLeft = '30px'; // 글자가 ⓒ를 침범하지 않게 밀어냄
      ...
      // 🔥 [핵심 수정] label과 remove 사이에 citationInput을 삽입합니다.
    ...
    }
    {
      ...
      for(const field of fieldItems) {
        ...
        if(field.type==='checkbox'){
          ...
          slider.setAttribute('aria-label',field.label+' 슬라이더');
  ...
  function open(sideValue,group,konvaNode,refresh=false){
    ...
    if(definition.formOptions?.desktopCategories){
      ...
      const select=document.createElement('select');select.setAttribute('aria-label','편집할 캐릭터');
      ...
      categories.setAttribute('aria-label','입력 항목');
  ...
  function renderMobile(preserveScroll=false){
    ...
    let tabRow=tabs;
    if(split){tabRow=document.createElement('div');tabRow.className='character-tab-row';tabRow.setAttribute('role','tablist');tabRow.setAttribute('aria-label','편집 대상');tabs.removeAttribute('role');tabs.removeAttribute('aria-label');tabs.classList.add('character-tabs-scroll');tabRow.append(tabs);scrollableTabs(tabs);}
