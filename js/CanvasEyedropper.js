export function createCanvasEyedropper(stage, stickers, waitForDraw) {
  ...
  function pick() {
    ...
    return new Promise(resolve => {
      ...
      controls.setAttribute('aria-label', '캔버스 색상 선택');
      const message = document.createElement('span');
      message.setAttribute('role', 'status');
      message.textContent = '캔버스에서 색을 선택하세요';
      const close = document.createElement('button');
      close.type = 'button'; close.textContent = '취소';
      ...
      async function getSnapshot() {
        ...
        const task = (async () => {
          ...
          if (!result.width || !result.height) throw new Error('캔버스가 아직 준비되지 않았어요.');
      ...
      function showError(error) {
        if (!finished) message.textContent = error.name === 'SecurityError'
          ? '외부 이미지의 색을 읽을 수 없어요. 이미지 파일을 직접 업로드해 주세요.'
          : '색을 읽지 못했어요. 다시 선택하거나 취소해 주세요.';
      }
      ...
      shield.addEventListener('pointerup', async e => {
        ...
        busy = true; message.textContent = '색을 가져오는 중이에요…';
        try {
          const source=await getSnapshot();
