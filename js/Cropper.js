...
export async function cropImage(file,position) {
  ...
  return new Promise((resolve,reject)=>{
    ...
    dialog.innerHTML='<h2>이미지 자르기</h2><div class="crop-preview"><img alt="자를 이미지"></div><div class="dialog-actions"><button type="button" data-zoom-out disabled>축소</button><button type="button" data-zoom-in disabled>확대</button><button type="button" data-fit disabled>전체 맞추기</button><button type="button" data-rotate disabled>90° 회전</button><button type="button" data-reset disabled>초기 상태로</button></div><p>전체 맞추기를 누르면 이미지가 잘리지 않게 들어갑니다. 남는 여백은 투명하게 저장됩니다.</p><div class="dialog-actions"><button type="button" data-cancel>취소</button><button type="button" data-apply class="primary" disabled>적용</button></div>';
